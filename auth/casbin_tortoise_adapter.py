...
class CasbinRule(models.Model):
    ...
    v0 = fields.CharField(max_length=255, null=True)
    v1 = fields.CharField(max_length=255, null=True)
    v2 = fields.CharField(max_length=255, null=True)
    v3 = fields.CharField(max_length=255, null=True)
    v4 = fields.CharField(max_length=255, null=True)
    v5 = fields.CharField(max_length=255, null=True)
...
class Adapter(persist.Adapter):
    ...
    async def filter_query(self, querydb, filter):
        ...
        if len(filter.ptype) > 0:
            ret = await querydb.filter(ptype__in=filter.ptype).order_by(CasbinRule.pk)
            return ret
        if len(filter.v0) > 0:
            ret = await querydb.filter(v0__in=filter.v0).order_by(CasbinRule.pk)
            return ret
        if len(filter.v1) > 0:
            ret = await querydb.filter(v1__in=filter.v1).order_by(CasbinRule.pk)
            return ret
        if len(filter.v2) > 0:
            ret = await querydb.filter(v2__in=filter.v2).order_by(CasbinRule.pk)
            return ret
        if len(filter.v3) > 0:
            ret = await querydb.filter(v3__in=filter.v3).order_by(CasbinRule.pk)
            return ret
        if len(filter.v4) > 0:
            ret = await querydb.filter(v4__in=filter.v4).order_by(CasbinRule.pk)
            return ret
        if len(filter.v5) > 0:
            ret = await querydb.filter(v5__in=filter.v5).order_by(CasbinRule.pk)
            return ret
    ...
    async def _remove_policy(self, sec, ptype, rule):
        ...
        try:
            query = self._db_class.filter(ptype=ptype)
            filter_params = {}
            for i, v in enumerate(rule):
                filter_params[f'v{i}'] = v
            r = await query.filter(**filter_params).delete()
    ...
    async def _remove_filtered_policy(self, sec, ptype, field_index, *field_values):
        ...
        try:
            ...
            for i, v in enumerate(field_values):
                filter_params[f'v{field_index + i}'] = v
