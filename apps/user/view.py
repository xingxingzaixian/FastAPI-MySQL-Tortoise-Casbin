...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    return ResultResponse[schema.Token](data={'access_token': access_token, 'token_type': 'bearer'})
...
async def register(user: schema.UserCreate):
    ...
    return ResultResponse[schema.UserOut](data=user)
...
async def get_user_info(request: Request):
    return ResultResponse[schema.UserOut](data=request.state.user)
