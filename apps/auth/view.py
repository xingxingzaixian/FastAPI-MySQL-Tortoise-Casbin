...
async def add_role_perm(perm_info: RolePerm):
    ...
    e = await get_casbin()
    res = e.add_policy(perm_info.role, perm_info.model, perm_info.act)
    if res:
...
async def del_role_perm(perm_info: RolePerm):
    e = await get_casbin()
    res = e.remove_policy(perm_info.role, perm_info.model, perm_info.act)
    if res:
...
@router.post("/add/user/perm",
             ...
             response_model=ResultResponse[str],
             dependencies=[Depends(Authority('auth,add'))])
async def add_user_role(user_info: UserPerm):
    ...
    e = await get_casbin()
    res = e.add_permission_for_user(user_info.user, user_info.model,
                                    user_info.act)
    if res:
...
@router.post("/del/user/perm",
             ...
             response_model=ResultResponse[str],
             dependencies=[Depends(Authority('auth,del'))])
async def del_role_authority(user_info: UserPerm):
    e = await get_casbin()
    res = e.delete_permission_for_user(user_info.user, user_info.model,
                                       user_info.act)
    if res:
...
async def del_user_role(user_role: UserRole):
    ...
    e = await get_casbin()
    res = e.add_role_for_user(user_role.user, user_role.role)
    if res:
...
async def get_role_list(username: str):
    e = await get_casbin()
    result = e.get_roles_for_user(username)
